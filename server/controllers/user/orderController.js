...
//       appliedCoupons = [{ code: coupon.code, discount }];
...
//           orderId,
//         });
//         console.log("tracnsaction details after pushing to the db",wallet.transactions.transacrtionId);
...
exports.placeOrder = async (req, res) => {
  try {
    const { addressId, paymentMethod, cart, orderNotes, appliedCoupon } = req.body;
    const userId = req.user.id;
    ...
    if (appliedCoupon && appliedCoupon.code) {
      let coupon = await Coupon.findOne({ code: appliedCoupon.code.toUpperCase() });
      let isReferral = false;

      if (!coupon) {
        coupon = await ReferralCoupon.findOne({ code: appliedCoupon.code.toUpperCase() });
        console.log("referral code from the db",coupon.user.toString());
      ...
      if (coupon.status === "Expired" || coupon.usageCount >= coupon.usageLimit) {
        return res.status(400).json({ message: "This coupon has reached its usage limit or is expired" });
      }
      ...
      if (coupon.restrictions.newCustomersOnly) {
        const hasPreviousOrders = await Order.exists({ user: userId });
        if (hasPreviousOrders) {
          return res.status(400).json({ message: "This coupon is only for new customers" });
        }
      }
...
exports.getInvoice = async (req, res) => {
  try {
    ...
    const order = await Order.findOne({ orderId, user: userId }).populate("items.productId").populate("items.variantId");
    ...
    doc.fontSize(22).font('Helvetica-Bold').text("BLACK BEANS", { align: "center" });
    doc.fontSize(10).font('Helvetica').text("Premium Coffee Store", { align: "center" });
    doc.moveDown(0.5);
    ...
    doc.font('Helvetica').fontSize(10).text(`Invoice Number: INV-${order.orderId}`, 50, doc.y + 5);
    ...
    doc.font('Helvetica-Bold').fontSize(12).text("BILLED TO:", doc.page.width - 200, customerStartY);
    doc.font('Helvetica').fontSize(10).text(`${order.shippingAddress.fullname}`, doc.page.width - 200, doc.y + 5);
    doc.text(`Email: ${order.shippingAddress.email}`, doc.page.width - 200, doc.y + 5);
    doc.text(`Phone: ${order.shippingAddress.phone}`, doc.page.width - 200, doc.y + 5);
    ...
    tableHeaders.forEach((header, i) => {
      const textOptions = i === 0 ? { width: columnWidths[i] } : { width: columnWidths[i], align: 'right' };
    ...
    order.items.forEach((item) => {
      ...
      doc.text(quantity.toString(), 50 + columnWidths[0], itemY, { width: columnWidths[1], align: 'right' });
      doc.text(`${currencySymbol}${price.toFixed(2)}`, 50 + columnWidths[0] + columnWidths[1], itemY, { width: columnWidths[2], align: 'right' });
      doc.text(`${currencySymbol}${total.toFixed(2)}`, 50 + columnWidths[0] + columnWidths[1] + columnWidths[2], itemY, { width: columnWidths[3], align: 'right' });
    ...
    const summaryTableX = doc.page.width - summaryLabelWidth - summaryValueWidth - 50;
    ...
    addSummaryRow("Discount:", `-${currencySymbol}${order.discount.toFixed(2)}`);
    
    doc.moveTo(summaryTableX, doc.y)
    ...
    doc.text("Thank you for shopping with Black Beans. For any queries relating to your order, please contact our customer service.", 50, doc.y, {
      align: 'center',
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    const items = order.items.filter((item) => item.status == "none");
    let remainingItemsPrice = 0;
    for (const item of items) {
      if (item.variantId) {
        const variant = await Variant.findById(item.variantId);
        variant.stock += item.quantity;
        remainingItemsPrice += item.price;
        await variant.save();
      } else {
    ...
    wallet.balance += remainingItemsPrice;
    wallet.transactions.push({
      transactionId: nanoid(10),
      type: "credit",
      amount: remainingItemsPrice,
...
exports.cancelOrderItem = async (req, res) => {
  try {
    ...
    if (!["pending", "processing"].includes(order.status)) {
      return res.status(400).json({
        message:
          "Order items cannot be canceled after the order has been shipped",
      });
    }
    ...
    if (item.variantId) {
      ...
      if (product.variants.length > 0) {
        return res.status(400).json({
          message: `Product ${item.name} has variants; stock should be managed via variants`,
        });
      }
      product.stock += item.quantity;
      await product.save();
    ...
    const refundAmount = itemTotal + shippingCostPerItem;
    ...
    wallet.transactions.push({
      ...
      type: "credit",
      amount: refundAmount,
      description: `Partial refund: Cancelled item from Order #${orderId}`,
      orderId,
    });
    await wallet.save();
    ...
    // order.total = order.subtotal + order.shipping - (order.discount || 0);
