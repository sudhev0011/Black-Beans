...
//   const { _id } = req.params;
//   const { name, category, actualPrice, salePrice, stock, variants, updatedUrls, deletedImages, isFeatured, description } = req.body;
//   const files = req.files || [];
...
//       // Validate standalone fields if no variants
//       if (actualPrice === undefined || isNaN(actualPrice) || actualPrice < 0) {
//         return res.status(400).json({ success: false, message: "actualPrice is required and must be a numeric value >= 0 for products without variants" });
//       }
//       if (stock === undefined || isNaN(stock) || stock < 0) {
...
//       if (parsedVariants.length > 0) {
//         await Variant.deleteMany({ productId: _id }, { session });
//         const variantDocs = parsedVariants.map((variant) => ({
//           productId: _id,
//           size: variant.size,
//           unit: variant.unit,
...
//           stock: variant.stock,
//         }));
//         const createdVariants = await Variant.insertMany(variantDocs, { session });
//         product.variants = createdVariants.map((v) => v._id);
//         product.actualPrice = undefined; // Clear standalone fields if variants are used
...
//       const editedProduct = await product.save({ session });
...
//   try {
//     // Use _id from req.body instead of req.params
...
//     // Update or replace variants (without transaction)
//     if (parsedVariants.length > 0) {
//       await Variant.deleteMany({ productId: _id }); // Remove old variants
//       const variantDocs = parsedVariants.map((variant) => ({
//         productId: _id,
//         size: variant.size,
//         unit: variant.unit,
...
//         stock: variant.stock,
//       }));
//       const createdVariants = await Variant.insertMany(variantDocs); // Insert new variants
//       product.variants = createdVariants.map((v) => v._id);
//       product.actualPrice = undefined; // Clear standalone fields if variants are used
//       product.salePrice = undefined;
