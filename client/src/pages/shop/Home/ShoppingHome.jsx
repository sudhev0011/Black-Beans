...
import { useGetProductsQuery, useGetFeaturedProductsQuery, useGetCategoriesQuery } from '@/store/api/userApiSlice';
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <section className="featured-products bg-white px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="featured-products-grid grid grid-cols-2 gap-10 sm:grid-cols-3 lg:grid-cols-4" style={{ maxWidth: '1200px' }}>
            {featuredProductsData?.products?.map((product) => (
              <FeaturedProductCard
                ...
                price={product.variants.length > 0 ? (product.variants[0].salePrice ?? product.variants[0].actualPrice) : (product.salePrice ?? product.actualPrice)}
                actualPrice={product.variants.length > 0 ? product.variants[0].actualPrice : product.actualPrice}
                salePrice={product.variants.length > 0 ? product.variants[0].salePrice : product.salePrice}
      ...
      <section className="our-products px-4 py-5 bg-cover bg-center bg-no-repeat sm:px-6 lg:px-8">
        <div className="container mx-auto max-w-7xl">
          ...
          {productsLoading ? <div>Loading products...</div> : <ProductSlider products={productsData?.products} />}
        </div>
      </section>
