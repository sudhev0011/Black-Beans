...
import OtpVerificationModal from "./OtpVerify"
...
const EditProfileComponent = ({ onCancel }) => {
  ...
  const validateFields = () => {
    ...
    if (!formData.username.trim()) {
      ...
      errors.username = "Username should only contain letters, numbers, and spaces"
  ...
  const handleAvatarChange = (e) => {
    ...
    if (file) {
      ...
      if (!validTypes.includes(file.type)) {
        toast.info("Only JPG, PNG, or WEBP images are allowed");
        return;
      }
      if (file.size > 2 * 1024 * 1024) {
        // 2MB limit
        toast.info("Image size should be less than 2MB");
        return;
      }
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="relative">
                <Avatar className="h-32 w-32">
                  <AvatarImage src={avatarPreview || userData?.user?.image_url || "/placeholder.svg"} alt="Profile" />
