...
//         });
//       } else {
//         console.log("applied coupons before place order",appliedCoupons);
//         console.log("applied coupons before place order",orderData);
//         const result = await placeOrder(orderData).unwrap();
//         setOrderId(result.orderId);
//         setOrderComplete(true);
//         dispatch(clearCart());
...
const CheckoutComponent = () => {
  ...
  const [orderComplete, setOrderComplete] = useState(false);
  const [orderId, setOrderId] = useState(null);
  const [appliedCoupon, setAppliedCoupon] = useState(null); // Changed to single coupon
  ...
  const handleCouponChange = (coupon) => {
    setAppliedCoupon(coupon); // Set single coupon or null
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      dispatch(clearCart());
      navigate(
        `/user/checkout/order-failure/${error.data?.orderId || "unknown"}`,
        {
          state: {
            errorMessage:
